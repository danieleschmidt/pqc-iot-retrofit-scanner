...
class ScoringEngine:
    ...
    def __init__(self, config: Dict):
        ...
        self.multipliers = config['scoring'].get('security_multipliers', {})
    ...
    def calculate_composite_score(self, item: WorkItem) -> float:
        ...
        composite = (
            ...
            self.weights['technical_debt'] * norm_debt +
            self.weights['security'] * (1.0 if item.category == 'security' else 0.0)
        )
        
        # Apply category-specific boosts
        if item.category == 'security':
            composite *= self.thresholds['security_boost']
...
class ValueDiscoveryEngine:
    ...
    def select_next_best_value(self, items: List[WorkItem]) -> Optional[WorkItem]:
        ...
        for item in items:
            ...
            if item.composite_score < self.config['scoring']['thresholds']['min_score']:
                continue
            ...
            if self._assess_risk(item) > self.config['scoring']['thresholds']['max_risk']:
                continue
...
- **Min Score Threshold**: {self.config['scoring']['thresholds']['min_score']}
- **Security Boost**: {self.config['scoring']['thresholds']['security_boost']}x
- **Max Concurrent Tasks**: {self.config['execution']['validation'].get('max_concurrent_tasks', 1)}
